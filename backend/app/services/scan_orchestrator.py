...
def start_scan_job(request: ScanRequest, scan_source: str = "manual") -> int:
    """
    Initiates a scan job for the given request. (Synchronous version)
    """
    ...
    try:
        ...
        scan_job = models.ScanJob.create(
            ...
            status="running", created_at=datetime.utcnow(), scan_source=scan_source
        )
        scan_id = scan_job.id
        ...
        # --- Google Dork Workflow (THIS IS THE MISSING BLOCK) ---
        if data_type in ["phone", "ic"]:
